...
// Document is a named reference link (application guide, fee schedule, etc.)
type Document struct {
	Name string `json:"name" bson:"name"`
	URL  string `json:"url" bson:"url"`
}

// PermitFee holds the fee amount and unit for a permit type.
type PermitFee struct {
	Amount    *float64 `json:"amount" bson:"amount"`
	Unit      string   `json:"unit" bson:"unit"`
	Effective string   `json:"effective,omitempty" bson:"effective,omitempty"`
}

// Form is a downloadable form (PDF) required for a permit application.
type Form struct {
...
// RequiredForm links a Form to a rule, with mandatory/conditional status and a note.
...
type Permit struct {
	...
	Fee             *PermitFee     `json:"fee,omitempty" bson:"fee,omitempty"`
	Cost            float64        `json:"cost" bson:"cost"`
	CostNotes       string         `json:"costNotes,omitempty" bson:"costNotes,omitempty"`
	Documents       []Document     `json:"documents" bson:"documents"`
	RequiredForms   []RequiredForm `json:"requiredForms,omitempty" bson:"requiredForms,omitempty"`
	...
	URL             string         `json:"url" bson:"url"`
	ApplyOnlineURL  string         `json:"applyOnlineUrl,omitempty" bson:"applyOnlineUrl,omitempty"`
...
type PermitResponse struct {
	...
	Fee            *PermitFee     `json:"fee,omitempty"`
	Cost           float64        `json:"cost"`
	CostNotes      string         `json:"costNotes,omitempty"`
	Documents      []Document     `json:"documents"`
	RequiredForms  []RequiredForm `json:"requiredForms,omitempty"`
	ZoningRules    ZoningRules    `json:"zoningRules,omitempty"`
	Notes          string         `json:"notes,omitempty"`
	ApplyOnlineURL string         `json:"applyOnlineUrl,omitempty"`
